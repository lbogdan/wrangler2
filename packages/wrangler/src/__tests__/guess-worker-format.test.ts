...
describe("guess worker format", () => {
  ...
  it("logs a warning when a worker has exports, but not a default one", async () => {
    ...
    expect(std.warn).toMatchInlineSnapshot(
      `"The entrypoint index.ts has exports like an ES Module, but hasn't defined a default export like a module worker normally would. Assuming that it's a service-worker format worker."`
    );
  });
});
