    ...
    if (scriptExports.includes("default")) {
      ...
      console.warn(
        `The entrypoint ${path.relative(
          process.cwd(),
          entryFile
        )} has exports like an ES Module, but hasn't defined a default export like a module worker normally would. Assuming that it's a service-worker format worker.`
      );
